...
class Project:
    ...
    def from_root(cls, root):
        ...
        if "version" not in project:
            if "version" in project.get("dynamic", []) and "setuptools_scm" in pyproject_options.get("tool", {}):
                try:
                    import setuptools_scm
                    project["version"] = setuptools_scm.get_version(str(root), version_scheme=lambda v: str(v.tag), local_scheme=lambda x: "")
                    _setuptools_scm = True
                except ImportError:
                    raise _exceptions.PolycotylusUsageError(_exceptions.unravel("""
                    """))
            else:
