...
class OpenSUSE(BaseDistribution):
    ...
    def dockerfile(self):
        ...
        out = self._formatter(f"""
            ...
            FROM base AS build
            RUN zypper install -y which fish osc build perl-XML-Parser hostname python3-pip python3-setuptools awk
            ...
            # download (but not install) all the above packages. The seemingly
            # random exclusion below of `fips` is due to
            ...
            RUN zypper install -dfy rpm-build $(zypper search --installed-only | grep 'i. |' | grep -v openSUSE | grep -v fips | awk '{{ print $3 }}') python-rpm-macros update-alternatives {shlex.join(dependencies)}
