...
is reluctant and costly – Podman's rootless support gets ugly when given build
tools requiring multi-user setups. Please don't use Podman with `polycotylus` if
given the choice. Support is primarily intended for RedHat/RPM based Linux
distributions where Podman is favoured so strongly to the point of removing
Docker from the official package repositories.
