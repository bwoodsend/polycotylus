...
let's reset it to an empty file so that we can explore how each line was
derived. ::
...
Let's try our first build. We could start with any Linux distribution but I
always recommend doing your first build with Alpine since it's by far the
quickest and has the best diagnostics. ::
...
Let's run ``polycotylus alpine`` again and see where we get stuck next. After
installing some packages and making a lot of noise, you should find it errors
out when pip is trying to compile C extension whilst building the package.

.. code-block:: bash

    polycotylus._docker.Error: Docker command:
        $ docker run --rm --network=host --platform=linux/x86_64 -v/g/notebooks/polycotylus/examples/ubrotli/.polycotylus/alpine:/io:z -v/home/brenainn/.abuild/bwoodsend@gmail.com-63b087db.rsa:/home/user/.abuild/bwoodsend@gmail.com-63b087db.rsa:z -v/g/notebooks/polycotylus/examples/ubrotli/.polycotylus/alpine/3.18:/home/user/packages:z -t --user=1000 --ulimit nofile=1024:1048576 sha256:d9d05c5db0fbdb25ce94fac996f2ab1a8526b6504b04010de9084a3e9118633f sh -ec 'abuild -f'
    returned an error:
...
The next rebuild should carry you all the way to the end where you should get a message which looks like::
    ...
    Built 1 artifact:
    main: .polycotylus/alpine/3.18/x86_64/py3-ubrotli-0.1.0-r1.apk

That's the location of your package! You'll notice that it's got that ``3.18``
version number and the architecture ``x86_64`` in its path. That's because the
package we built is only compatible with Alpine v3.18.x and is compiled for
``x86_64``. Use the following syntaxes to target other versions and
architectures::
...
::
    ...
    error: Bad exit status from /var/tmp/rpm-tmp.GXShj3 (%build)

    RPM build errors:
        Bad exit status from /var/tmp/rpm-tmp.GXShj3 (%build)
