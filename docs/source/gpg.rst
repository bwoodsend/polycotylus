...
Some distributions optionally use GnuPG_ to sign their packages. These
distributions are Arch, Fedora, Manjaro, OpenSUSE. The other distributions
either use their own wrappers around OpenSSL, for which the signing process is
documented under :ref:`each distribution's quirks page <building for>`, or don't
meaningfully support signing.

.. note::

    Before embarking on signing, bear in mind that, without an in-person or web
    of trust based public key verification, a signature is more or less
    meaningless, providing less security than HTTPS.
